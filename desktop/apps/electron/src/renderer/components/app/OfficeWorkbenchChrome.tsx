...
import { Tooltip, type TooltipProps } from '@/components/amphi/Tooltip'
import { cn } from '@/lib/cn'

type OfficeTooltipOptions = Pick<TooltipProps, 'appearance' | 'delayMs'>

/** Shared application chrome; each editor retains ownership of its document lifecycle. */
...
export function OfficePanelControls({ closeLabel, expanded, expandLabel, onClose, onToggleExpanded, testIdPrefix, toggleTestId, tooltipOptions }: {
  closeLabel: string
  expanded: boolean
  expandLabel: string
  onClose?: () => void
  ...
  testIdPrefix: string
  toggleTestId?: string
  tooltipOptions?: OfficeTooltipOptions
}) {
  return (
    <>
      {onToggleExpanded ? (
        <Tooltip {...tooltipOptions} content={expandLabel} placement="bottom">
          <button
      ...
      {onClose ? (
        <Tooltip {...tooltipOptions} content={closeLabel} placement="bottom">
          <button
...
export function OfficeDocumentTabs({ actions, activeId, icon, label, newDisabled, newIcon, newLabel, onClose, onCreate, onSelect, tabs, testIdPrefix, tooltipOptions }: {
  actions?: ReactNode
      ...
      <div aria-label={label} className="flex h-full min-w-0 flex-1 items-center gap-1 overflow-x-auto" data-testid={`${testIdPrefix}-document-tabs`} role="tablist">
        {tabs.map((tab) => (
          <div
            ...
            key={tab.id}
          >
            <Tooltip {...tooltipOptions} content={tab.label} placement="bottom">
              <button
            ...
            <Tooltip {...tooltipOptions} content={tab.closeLabel} placement="bottom">
              <button
        ...
        <Tooltip {...tooltipOptions} content={newLabel} placement="bottom">
          <button
            aria-label={newLabel}
