...
export function ExcelFormulaWizardDialog({
  ...
  const copy = {
    addArgument: t('excel.formula.dialog.addArgument'),
    all: t('excel.formula.dialog.all'),
    back: t('excel.formula.dialog.back'),
    cancel: t('excel.formula.dialog.cancel'),
    calculating: t('excel.formula.dialog.calculating'),
    categories: {
      common: t('excel.formula.dialog.categories.common'),
      financial: t('excel.formula.dialog.categories.financial'),
      logical: t('excel.formula.dialog.categories.logical'),
      text: t('excel.formula.dialog.categories.text'),
      date: t('excel.formula.dialog.categories.date'),
      lookup: t('excel.formula.dialog.categories.lookup'),
      math: t('excel.formula.dialog.categories.math'),
      statistical: t('excel.formula.dialog.categories.statistical'),
    },
    chooseRange: t('excel.formula.dialog.chooseRange'),
    description: t('excel.formula.dialog.description'),
    donePicking: t('excel.formula.dialog.donePicking'),
    empty: t('excel.formula.dialog.empty'),
    functions: t('excel.formula.dialog.functions'),
    insert: t('excel.formula.dialog.insert'),
    next: t('excel.formula.dialog.next'),
    optional: t('excel.formula.dialog.optional'),
    parameters: t('excel.formula.dialog.parameters'),
    pickDetail: t('excel.formula.dialog.pickDetail'),
    preview: t('excel.formula.dialog.preview'),
    required: t('excel.formula.dialog.required'),
    search: t('excel.formula.dialog.search'),
    target: t('excel.formula.dialog.target'),
    title: t('excel.formula.dialog.title'),
  }
    ...
    const timeout = window.setTimeout(() => {
      ...
      void onEvaluate(formula).then((result) => {
        ...
        if (evaluationId.current === id) setPreview({
          status: 'error',
          message: t('excel.formula.previewUnavailable'),
        })
      })
    }, 220)
    return () => window.clearTimeout(timeout)
  }, [formula, locale, missingIndex, onEvaluate, selected, wizardOpen, t])
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="min-h-0 flex-1 overflow-y-auto p-5">
              ...
              <div aria-live="polite" className={`mt-4 rounded-lg border px-3 py-2 ${preview.status === 'error' ? 'border-status-warning/30 bg-status-warning/8' : 'border-border-subtle bg-bg-app'}`}>
                ...
                {missingIndex !== null ? <p className="mt-1 text-[11px] text-status-warning">{t('excel.formula.completeRequiredArgument', { name: parameters[missingIndex]?.name })}</p> : null}
                {missingIndex === null && preview.status === 'calculating' ? <p className="mt-1 text-[11px] text-text-secondary">{copy.calculating}</p> : null}
                {preview.status === 'success' ? <p className="mt-1 break-words font-mono text-[12px] font-semibold text-emerald-600">{preview.value || (t('excel.formula.blank'))}</p> : null}
          ...
          <>
            <div className="flex min-h-0 flex-1">
              <div className="flex w-72 shrink-0 flex-col border-r border-border-subtle p-4">
                ...
                <select aria-label={t('excel.formula.category')} className="mt-2 h-8 rounded-md border border-border-subtle bg-bg-app px-2 text-[11px] text-text-secondary outline-none focus:border-emerald-500" onChange={(event) => setCategory(event.target.value as FormulaCategoryFilter)} value={category}><option value="all">{copy.all}</option>{(Object.entries(copy.categories) as Array<[ExcelFormulaCategory, string]>).map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select>
                <div className="mt-3 flex items-center justify-between text-[10px] text-text-tertiary"><span>{matches.length} {copy.functions}</span>{recentFunctions.length > 0 ? <span>{t('excel.formula.recentFirst')}</span> : null}</div>
              ...
              <div className="min-w-0 flex-1 overflow-y-auto p-5">
                {selected ? <><div className="flex items-center gap-2"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-emerald-500/10 font-serif text-lg italic text-emerald-600">fx</span><div className="min-w-0"><h3 className="font-mono text-base font-semibold text-text-primary">{selected.name}</h3><p className="mt-0.5 break-words font-mono text-[11px] text-emerald-600">={selected.syntax}</p></div></div><p className="mt-4 text-[12px] leading-6 text-text-secondary">{selected.description}</p><h4 className="mt-5 text-[11px] font-semibold text-text-primary">{copy.parameters}</h4>{selected.parameters.length > 0 ? <div className="mt-2 space-y-2">{selected.parameters.map((parameter, index) => <div className="rounded-lg border border-border-subtle bg-bg-app px-3 py-2" key={`${parameter.key}:${index}`}><p className="font-mono text-[11px] font-medium text-emerald-600">{parameter.name}<span className="ml-1.5 font-sans text-[9px] text-text-tertiary">{parameter.required ? copy.required : copy.optional}</span></p><p className="mt-1 text-[10px] leading-5 text-text-tertiary">{parameter.detail}</p></div>)}</div> : <p className="mt-2 text-[11px] text-text-tertiary">{t('excel.formula.noArguments')}</p>}</> : <p className="text-[11px] text-text-tertiary">{copy.empty}</p>}
              </div>
            </div>
