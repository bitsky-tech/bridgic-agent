...
`pptxgenjs` is NOT bundled — run `npm install pptxgenjs` on first use in a session, then `require('pptxgenjs')`. It persists for the rest of that session, so later scripts require it directly. The app ships its own Node and npm, so this works even when the machine has no system Node. The model knows the API; these are the footguns:
...
`pptxgenjs` (npm, not bundled — install on first use in a session) · `markitdown[pptx]`, `Pillow`, `defusedxml`, `lxml` (pip — text dump, thumbnail, clean, validate) · LibreOffice (`soffice`, auto-configured for sandboxed environments via `scripts/office/soffice.py`) · `pdftoppm` (Poppler)
