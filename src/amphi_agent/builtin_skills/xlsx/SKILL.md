...
> `openpyxl`, `pandas`, and `markitdown` are NOT bundled with this app — `pip install` them into the session environment on first use. They persist for the rest of that session, so later scripts import directly; reinstall only if an import fails (or the `markitdown` command is missing).
...
`openpyxl`, `pandas`, `markitdown` (pip, not bundled — install on first use in a session) · LibreOffice (`soffice`, auto-configured for sandboxed environments via `scripts/office/soffice.py`)
