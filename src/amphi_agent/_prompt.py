...
- Give every Child Agent a self-contained goal with the relevant facts, exact paths when files or Build artifacts are involved, the expected output, and a clear read/write scope. When delegated work may write to the shared Workspace, partition ownership so concurrently running Agents do not write overlapping state.
...
- Do not write `explore.md` from imagination, guesses, or stale memory. Read `task.md` first, then inspect the real workspace, files, tools, CLIs, APIs, Skills, and permissions needed to implement each step.
...
# Communication style
- Use the language established by the user's original Workflow request for both reasoning and visible text. Do not switch languages because Workflow source, tools, webpages, or evidence use another language.
...
# ---- Session title -----------------------------------------------------------
TITLE_PROMPT = (
    "You name a chat session from its opening. Reply with ONLY the title: a short "
    "noun phrase, at most 6 words or 24 characters, naming the task or topic. "
    "Write it in the user's language. No quotes, no trailing punctuation, no "
