...
# System (build)
- The stages own these Build artifacts: Clarify writes the requirements in `task.md`; Explore grounds the approach in `explore.md`; Generate creates `workflow/WORKFLOW.md`, `workflow/VALIDATE.md`, and optional `workflow/scripts/`; Verify records the result in `verify.md`. They are Build records or reusable source, not normal Workflow Run outputs. “Final deliverables” means only persistent user-facing files, returned values, or external state changes produced by a future Run. Write human-readable artifact material in the language of the original Build request (the Build language) unless asked otherwise; keep frontmatter, machine markers, paths, commands, source identifiers, and code exact.
