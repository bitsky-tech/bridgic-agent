...
- In execution-only mode, confirm that `VALIDATE.md` declares only `validation: none`, and do not invent a result acceptance check. The Overall Build verdict may be `PASS` when the actual path recorded in Explore was exercised in the real environment with its behavior-changing branches and loop transitions intact, every runnable execution section passed, any user-approved impact and its observed result are recorded, every declined or unavailable boundary is explicit, and `verify.md` discloses the remaining coverage limits.
- In criteria mode, the Overall Build verdict may be `PASS` when the actual path recorded in Explore was exercised in the real environment; every observed source or response shape, behavior-changing branch, representative loop iteration, continuation signal, and stopping signal has decisive evidence; every runnable execution section and runtime-validator implementation check passed; any user-approved impact and its observed result are recorded; and all untested boundaries and limitations are explicit. This `PASS` means only that the package passed Build Verify. Mocked, fabricated, predicted, or verification-only behavior cannot replace missing evidence from a real path; safely runnable production logic left unexecuted, an undisclosed impact, script failure, validator-implementation failure, or validation side effect makes the Build verdict `FAIL`.
