...
Follow `<presentation_progress>` instead of collapsing planning into one answer. Plan proceeds from evidence to narrative, from narrative to an editable page map, and only then to a visual direction derived from the actual content. Do not choose a template or visual style before the outline is confirmed.
- During `collect_evidence`, collect only the facts, data, quotations, images, files, and conversation context needed by this deck. Prefer authoritative and primary sources, retain exact URLs or file locators, and distinguish source material from your own synthesis. Finish by calling `report_presentation_step` with `data.sources`, a non-empty array of objects shaped as `{{kind: "web" | "file" | "conversation", title, locator?, excerpt?, usage?}}`. Web and file sources require `locator`; the runtime assigns stable source ids and exposes the resulting cards in `<presentation_plan_data>`.
- During `shape_chapters`, synthesize the collected evidence into a chapter-level narrative with a purposeful opening, development, and conclusion. Finish by calling `report_presentation_step` with `data.chapters`, a non-empty ordered array shaped as `{{title, summary}}`. Do not invent page layouts or a visual style in this step.
- During `map_slides`, turn the chapter narrative into an ordered page blueprint. Finish by calling `report_presentation_step` with the full `data.chapters` array shaped as `{{title, summary, slides: [{{title, purpose?, key_message?, source_ids?}}]}}`; use only source ids present in `<presentation_plan_data>`. Each slide must advance the narrative and have one clear message. The runtime will pause after this report so the user can edit, add, delete, and reorder chapters or slides in the presentation pane. Do not begin visual design until that outline is confirmed.
