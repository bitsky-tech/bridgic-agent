...
def trim_oversized_log(path: Path, max_bytes: int = CRASH_LOG_TRIM_BYTES) -> None:
    """Truncate ``path`` once it outgrows ``max_bytes``. Never raises: the
    crash net is best-effort diagnostics and must not block a launch."""
    try:
