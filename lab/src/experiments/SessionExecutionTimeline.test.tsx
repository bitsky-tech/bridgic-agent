...
describe('session execution timeline', () => {
  ...
  test('compact debug links belong to the correct turn and omit unreached stages', () => {
    ...
    const turns = html.split(/<li class="session-turn" data-turn-id="[^"]+">/).slice(1)
    expect(turns[0]).not.toContain('earlier-brief execution summary')
    expect(turns[0]).not.toContain('earlier-brief stage decision')
    expect(turns[0]).toContain('Model response data is unavailable')
    expect(turns[0]).toContain('data-debug-call-id="earlier-brief-call"')
    expect(turns[0]).not.toContain('later-brief')
    expect(turns[1]).not.toContain('later-brief execution summary')
    expect(turns[1]).not.toContain('later-brief stage decision')
    expect(turns[1]).toContain('Model response data is unavailable')
